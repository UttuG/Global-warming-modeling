...
import Global_warming_model

class TestGlobal_warming_model(unittest.TestCase):
	...
	def test_generate_models(self):
		...
		degrees = [1]
		models = Global_warming_model.generate_models(x, y, degrees)
	...
	def test_r_squared(self):
		...
		r_sq = Global_warming_model.r_squared(y, est)
		...
		r_sq = Global_warming_model.r_squared(y, y)
		self.assertIsInstance(r_sq, float, "r_squared should return a float")
		self.assertEquals(r_sq, 1.0)

	def test_gen_cities_avg(self):
		# test for just one city
		climate = Global_warming_model.Climate('data.csv')
		test_years = pylab.array(Global_warming_model.TESTING_INTERVAL)
		result = Global_warming_model.gen_cities_avg(climate, ['SEATTLE'], test_years)
		...
		# multiple cities
		result = Global_warming_model.gen_cities_avg(climate, Global_warming_model.CITIES, test_years)
		...
		# years range
		# multiple cities
		result = Global_warming_model.gen_cities_avg(climate, ['TAMPA', 'DALLAS'], test_years)
	...
	def test_moving_avg(self):
		...
		result = Global_warming_model.moving_average(y, window_length)
	...
	def test_rmse(self):
		...
		result = Global_warming_model.rmse(pylab.array(y), pylab.array(estimate))
	...
	def test_gen_std_devs(self):
		climate = Global_warming_model.Climate('data.csv')
		years = pylab.array(Global_warming_model.TRAINING_INTERVAL)
		result = Global_warming_model.gen_std_devs(climate, ['SEATTLE'], years)
		...
		result = Global_warming_model.gen_std_devs(climate, Global_warming_model.CITIES, years)
		...
		result = Global_warming_model.gen_std_devs(climate, ['TAMPA', 'DALLAS'], years)
...
if __name__ == '__main__':
    ...
    suite.addTest(unittest.makeSuite(TestGlobal_warming_model))
